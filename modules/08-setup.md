...
**bge-base-zh-v1.5 安装**（模型放运行时目录，技能更新不覆盖，幂等可反复跑）：

```bash
# 推荐：一键全流程（依赖 + 模型 + 初始化 + opencode 配置，幂等）
python3 ~/.local/share/yishi/scripts/install.py
# 仅模型（--model-only，跳过已装依赖与已初始化数据）
...
> 模型下载用 urllib 内建实现（重试 4 次），不依赖 curl 或 wget——Windows 无 curl.exe 亦可正常下载。
