...
**产出**：单文件自包含交互式脑图（无外网依赖），径向树布局：根「忆时记忆(N)」→ 第1层 记忆类型（决策/任务/偏好/…，各色圆点）→ 第2层 主题聚类（与 viz 之 TOPICS 同规则）→ 第3层 记忆叶子。交互：滚轮缩放、拖动平移、点节点展开/收起、悬停看摘要/全内容、底部按钮全部展开/收起/归位。

**依赖**：同目录 mindmap_template.html（含 `/*__MINDMAP_DATA__*/` 占位符）；上一层 memory_core.py。须设 `MEMO_DIR`。
