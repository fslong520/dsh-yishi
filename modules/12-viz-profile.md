...
**产出**：单文件自包含交互式**网状图谱**（无外网依赖，D3 力导向 Canvas，仿 MPE graph-view）。数据：节点=全部记忆，连线=语义关联（Chroma `relationships` 集合，score≥0.5）。节点**按记忆类型着色**（决策⚖️/任务📋/偏好⭐/上下文📌/技能🔧/时间⏰/情绪💜），**大小 ∝ 记忆容量(content_len)×频次(freq)**，叠加关系度。节点标注**简洁 title**（存储时自动生成，旧记忆回退从首句取）。交互：滚轮缩放、空白拖动平移、拖动节点、**悬停浮窗看完整内容**、点击节点高亮邻接1~3层(BFS)+看详情、不相关淡出、搜索过滤、全局/局部（以当前节点为中心）切换、图例分型。
