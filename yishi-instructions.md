...
2【值必存】凡本轮出现新的 决策/偏好/任务完成/情绪波动/时间节点 之任一，
   ...
   **凡 store 不带 --merge-ids 亦不带 --force 者，判为违规直存。**
   ...
   第二段【store 决策】先判**从属关系**，再定操作：
   · **判下挂**（同一大事情的子事情/细节/附属/分项）→ --parent 挂为子节点：
     python3 $YISHI store --type <类型> --keywords "k1,k2" --emotion <0.0~1.0> \
       --parent "<父节点ID>" "<内容>"
     （如"项目的某个子任务""技能的某个用法"——挂到该大事/技能入口下，
       不合并，保留独立条，但建立从属关系；见 🔗 可选父节点提示）
   · **判合并**（同一件事，与旧忆相似度高、仅改一点/补充进展）→ --merge-ids：
     python3 $YISHI store --type <类型> --keywords "k1,k2" --emotion <0.0~1.0> \
       --merge-ids "<候选ID1>,<候选ID2>" "<综合合并版全文>"
     （同一任务修复/同一铁律/同一项目进展，多次迭代 → 合并成一条权威版）
   · **判独立**（不同主题，仅字面近似，无从属无同主题）→ 机械存储：
     python3 $YISHI store --type <类型> --keywords "k1,k2" --emotion <0.0~1.0> \
       --merge-ids "" "<完整内容>"
   · 判定逻辑：**大事情的子事情 → 下挂；同事情相似度高仅小改 → 合并；无关 → 独立**。
     三条里两条强相关就并两条；子事情勿合并进父（层次不同，下挂保树）。
...
5【技必查】遇任务先查技能记忆，命中即按技能执行：
   ...
   · **因果树（2026-09-02 定版）：父子即因果——父=前因，子=后果；果复为因，链式传导。**
     挂载语义：--parent = 认因（本条为父之果）；若本条又是他事之因，他事之果续挂本条下。
     同一词条勿使因果混论：一条记忆只有一个父（一因），可有多子（多果）；禁止成环
     （demote/promote 已防环——目标父是本条子代即拒）。
   · 链式前向星：技能入口用 --parent 挂载子节点（用法/素材/文件），
     promote（因果上浮）升到祖父层/根——本条升为更上游之因；demote（因果下沉）挂到指定父下成为其果
   · **存储即因果树**：store 时若见 🔗 可选因节点，先判本内容是哪个事件之果
     → 用 --parent 挂其下成链（勿平铺散存）；无合适因则独立存（自为因，待果来挂）
   · 上浮/下沉由 AI 决策（脚本仅提示）：recall 输出 🏔 层级 + 🔺因链/🔻果（因果链），
     见 🔥 热记忆沉深层 → 主动 promote；见 ❄️ 冷根节点 → 主动 demote。
     查找用渐进式：recall 命中 → 🔺因链溯源、🔻果链下探（--tree 顺因果展开），逐步深入。
     决策依据=检索热度+因果位置，判断后执行命令，并在回答说明理由。
